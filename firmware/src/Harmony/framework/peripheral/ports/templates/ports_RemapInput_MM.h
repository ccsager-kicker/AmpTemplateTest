...
#ifndef _PORTS_REMAPINPUT_MM_H
#define _PORTS_REMAPINPUT_MM_H
...
PLIB_TEMPLATE void PORTS_RemapInput_MM( PORTS_MODULE_ID      index , PORTS_REMAP_INPUT_FUNCTION inputFunction , PORTS_REMAP_INPUT_PIN      remapInputPin )
{
	uint32_t position = (inputFunction & 0x0F) * 8;
	uint32_t mask = 0x1F << position;
	uint8_t offset = (inputFunction & 0xF0) >> 4;
    *(&RPINR1 + offset) = ( ( *((SFR_TYPE *)(&RPINR1 + offset)) ) & ~(mask) ) | ( (mask) & ((offset)<<(position)) );
}
...
PLIB_TEMPLATE bool PORTS_ExistsRemapInput_MM( PORTS_MODULE_ID index )
{
    return true;
}
